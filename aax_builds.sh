...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    cmake --build build-aax --config $build_config -j12 --target Chameleon_AAX | xcpretty

else # Windows
    cmake -Bbuild-aax -G"Visual Studio 16 2019" -A x64
    cmake --build build-aax --config $build_config --parallel $(nproc) --target Chameleon_AAX
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    wraptool verify --verbose --in $aax_location/Contents/x64/Chameleon.aaxplugin
fi

# reset AAX SDK field...
#sed_cmakelist "s~juce_set_aax_sdk_path.*~# juce_set_aax_sdk_path(NONE)~"

rm -rf "$aax_target_dir/Chameleon.aaxplugin"
cp -R "$aax_location" "$aax_target_dir/Chameleon.aaxplugin"

if [[ "$*" = *deploy* ]]; then
    set +e

    ssh "smartguitarml@gmail.com" "rm -r ~/aax_builds/${TARGET_DIR}/Chameleon.aaxplugin"
